...
public class CommonItemMeta implements Serializable
{
	...
	public View getListItemView(LayoutInflater inflater)
	{
		...
		case IMG_CONTENT:
			{
				...
				if(!ValidatorUtil.isEmpty(titleImage))
				{
					LoadImageUtil.getInstance().loadImage(img, titleImage);
				}
			}
		...
		case LARGE_IMG:
		{
			view = inflater.inflate(MispCommonIDName.layout_misp_list_item_imgbg, null);
			TextView title_view = (TextView) view.findViewById(MispCommonIDName.item_imgbg_name);
			ImageView img = (ImageView) view.findViewById(MispCommonIDName.item_imgbg_img);
			title_view.setText(title);
			if(!ValidatorUtil.isEmpty(titleImage))
			{
				LoadImageUtil.getInstance().loadImage(img, titleImage);
